...
def calcular_distancia_duracion_indicaciones(origen, destino, key, modo_transporte):
    ...
    route_params = {
        ...
        "vehicle": modo_transporte,  # Modo de transporte seleccionado por el usuario
        "key": key,
...
# Bucle principal para solicitar origen, destino y modo de transporte hasta que el usuario escriba "S"
while True:
    origen = input("Ingrese el origen (o escriba 'S' para terminar): ")
    if origen.lower() == "s":
        break
    destino = input("Ingrese el destino (o escriba 'S' para terminar): ")
    if destino.lower() == "s":
        break

    modo_transporte = input("Ingrese el modo de transporte (car, foot, bike, etc.): ")

    # Calcular la distancia, duración y obtener las indicaciones entre el origen y el destino proporcionados por el usuario
    distancia, duracion, instrucciones = calcular_distancia_duracion_indicaciones(origen, destino, key, modo_transporte)
